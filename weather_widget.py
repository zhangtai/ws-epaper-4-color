...
def add_calendar_block(draw):
    ...
    # Draw the left side
    draw.text(
        (100, 50), weekday, font=large_font, fill=(255, 0, 0)
    )  # Weekday in Chinese (e.g., 周二)
    draw.text(
        (300, 50), month, font=large_font, fill=(192, 192, 192)
    )  # Month in Chinese (e.g., 10月)

    draw.text((100, 150), day, font=x_large_font, fill=(0, 0, 0))  # Day (e.g., 15)
